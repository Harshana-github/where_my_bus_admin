...
const TaskStatusPage = () => {
  ...
  const taskStatuses = useTaskStatusStore((state) => state.taskStatuses);
  const getAllTaskStatuses = useTaskStatusStore(
    (state) => state.getAllTaskStatuses
  );
  const addTaskStatus = useTaskStatusStore((state) => state.addTaskStatus);
  const updateTaskStatus = useTaskStatusStore(
    (state) => state.updateTaskStatus
  );
  const deleteTaskStatus = useTaskStatusStore(
    (state) => state.deleteTaskStatus
  );
  ...
  const handleRowInserting = async (e) => {
    const result = await addTaskStatus(e.data);
    if (result.isOk) {
      notify(t("settings.taskStatus.messages.add_success"), "success", 2000);
      await getAllTaskStatuses();
    } else {
      notify(t("settings.taskStatus.messages.add_error"), "error", 2000);
    }
  };
  ...
  const handleRowUpdating = async (e) => {
    const updatedData = {
      id: e.oldData.id,
      task_status_name:
        e.newData.task_status_name ?? e.oldData.task_status_name ?? "",
    ...
    const result = await updateTaskStatus({
      id: e.key,
      taskStatus: updatedData,
    });
    if (result.isOk) {
      notify(t("settings.taskStatus.messages.update_success"), "success", 2000);
      await getAllTaskStatuses();
    } else {
      notify(t("settings.taskStatus.messages.update_error"), "error", 2000);
    }
  };

  const handleRowRemoving = async (e) => {
    const result = await deleteTaskStatus(e.key);
    if (result.isOk) {
      notify(t("settings.taskStatus.messages.delete_success"), "success", 2000);
      await getAllTaskStatuses();
    } else {
      notify(t("settings.taskStatus.messages.delete_error"), "error", 2000);
    }
  };
    ...
    <div className={"task-status-page"}>
      ...
      >
        ...
        <Editing mode="popup" allowUpdating allowDeleting allowAdding useIcons>
          <Popup
            title={t("settings.taskStatus.messages.add_edit_task_status")}
            showTitle={true}
            width={500}
            height={300}
          />
          <Form colCount={1}>
            <Item itemType="group" colCount={1}>
              ...
              <Item dataField="task_status_name" />
        ...
        <Column
          caption={t("settings.taskStatus.fields.status_name")}
          dataField="task_status_name"
          dataType="string"
          alignment="left"
        >
          <RequiredRule
            message={t("settings.taskStatus.messages.status_name_required")}
          />
        </Column>
